...
class ForcePointLoad(ElementLoad):
    ...
    Methods:
        __init__(element, x, PX=0.0, PY=0.0, PZ=0.0):
            Initializes the PointLoad object with the given parameters.
        point_load(f, L, x):
            Computes the internal forces and moments generated by the point load.
        __call__(x):
            Returns the load components (PX, PY, PZ) as a NumPy array.
    Raises:
...
class MomentPointLoad(ElementLoad):
    ...
    Methods:
        point_load(M, L, x):
            Calculates the equivalent nodal forces and moments for a given moment 
            applied at a specific position along the element.
        __call__(x):
            Returns the moments (MX, MY, MZ) as a NumPy array.
    Raises:
        ValueError: If the position `x` is not within the range [0, L].
    """
...
class DistributedLoad(ElementLoad):
    ...
    Methods:
        __call__(x):
            Returns the distributed load components as a numpy array.
        distributed_load(w, L):
            Computes the equivalent nodal forces and moments for a distributed load.
    Parameters:
        element (Element): The structural element to which the load is applied.
    ...
    def __init__(self, element, WX=0.0, WY=0.0, WZ=0.0, axis='local'):
        """
        Initializes a load object applied to a structural element.
        Parameters:
            element (object): The structural element to which the load is applied.
                              It must have attributes `L` (length of the element)
                              and `r` (rotation matrix for coordinate transformation).
            WX (float, optional): Distributed load in the X direction. Defaults to 0.0.
            WY (float, optional): Distributed load in the Y direction. Defaults to 0.0.
            WZ (float, optional): Distributed load in the Z direction. Defaults to 0.0.
            axis (str, optional): Specifies whether the load is defined in the 'local'
                                  or 'global' coordinate system. Defaults to 'local'.
        Attributes:
            element (object): The structural element associated with the load.
            WX (float): Distributed load in the X direction.
            WY (float): Distributed load in the Y direction.
            WZ (float): Distributed load in the Z direction.
        Notes:
...
class TemperatureLoad(ElementLoad):
    ...
    Methods:
        __call__(x):
            Returns a zero vector for the load distribution at a given position `x`.
    Notes:
    ...
    def __init__(self, element, delta_Tx, delta_Ty, delta_Tz, hy, hz, alpha=None):
        ...
        Attributes:
            element (Element): The structural element associated with the thermal load.
            dicy (dict): Dictionary containing the calculated thermal loads and moments:
                - 'PXj', 'PXk': Axial forces due to temperature change in the x-direction.
                - 'PYj', 'PYk': Axial forces in the y-direction (set to 0).
                - 'PZj', 'PZk': Axial forces in the z-direction (set to 0).
                - 'MYj', 'MYk': Moments about the y-axis due to temperature change in the z-direction.
                - 'MZj', 'MZk': Moments about the z-axis due to temperature change in the y-direction.
        """
...
class NodeLoad():
    ...
    Args:
        node (object): The node to which the load is applied.
        PX (float, optional): Force in the X direction. Default is 0.0.
        PY (float, optional): Force in the Y direction. Default is 0.0.
        PZ (float, optional): Force in the Z direction. Default is 0.0.
        MX (float, optional): Moment about the X axis. Default is 0.0.
        MY (float, optional): Moment about the Y axis. Default is 0.0.
        MZ (float, optional): Moment about the Z axis. Default is 0.0.
    """
...
class NodeLoad2D():
    ...
    Args:
        node (object): The node to which the load is applied.
        PX (float, optional): Force in the X direction. Defaults to 0.0.
        PY (float, optional): Force in the Y direction. Defaults to 0.0.
        MZ (float, optional): Moment about the Z axis. Defaults to 0.0.
    """
...
class Spring2D():
    ...
    Methods:
        __init__(node, KX=0.0, KY=0.0, KMZ=0.0):
            Initializes the Spring2D object with a node and stiffness values for X, Y, and Z directions.
    Parameters:
...
class Spring(NodeLoad):
    ...
    Args:
        ...
        KX (float, optional): Stiffness in the X translational direction. Defaults to 0.0.
        KY (float, optional): Stiffness in the Y translational direction. Defaults to 0.0.
        KZ (float, optional): Stiffness in the Z translational direction. Defaults to 0.0.
        KMX (float, optional): Stiffness in the X rotational direction. Defaults to 0.0.
        KMY (float, optional): Stiffness in the Y rotational direction. Defaults to 0.0.
        KMZ (float, optional): Stiffness in the Z rotational direction. Defaults to 0.0.
    """
